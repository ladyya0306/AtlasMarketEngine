...
2. 如果要对照公开版保留的市场验证口径，用户应该怎么输入才能逐组复现。  
...
## 4.4 公开版当前可以对外怎么讲“成交量走势”和“A/B 分区成交单价走势”

公开版虽然不保留完整原始批次 `.db` 包，但保留了内部正式重确认批次派生出来的趋势摘要。  
就目前这版公开仓库而言，可以稳定讲清下面两类结果：

### 成交量走势

按 `V1 / V2 / V3` 三组汇总后的 3 个月成交量如下：
...
1. 三组在 month 1 都没有形成最终成交，这说明系统里的成交不是“激活即成交”，而是要经过候选、谈判、下单和交割展开。
2. `V3` 在 month 2 放量最强，说明卖方环境的需求压力更早转成真实成交。
3. `V2` 的成交高点更偏向 month 3，说明买方环境的兑现更慢、更分散。
...
1. `A` 区单价始终显著高于 `B` 区，说明核心区和外围区的价格层级没有被模拟过程打散。
2. `V3` 在 month 2、month 3 都出现 `B` 区成交，说明卖方环境下需求开始向外围区外溢。
3. `V2` 的 `B` 区成交更少，说明买方环境里买家更容易停留在高匹配度房源，而不是迅速扩散到外围区。

如果对外需要一个简洁说法，可以直接概括为：

**公开版已经能够展示“成交量时滞 + A/B 分区价格层级 + 卖方环境外溢更早出现”这三类结果。**

---

## 5. 如何用 Scholar CLI 复现公开版口径
...
当前公开版默认用 3 个月，不是随便拍脑袋定的，而是基于当前阶段的项目边界：

1. month 1 往往更多在形成候选、谈判和订单，而不是立刻形成最终成交。
2. month 2 开始更容易看到真实成交放量。
3. month 3 已经能观察到 A/B 分区的成交量和成交均价差异。
4. 如果一上来就把 6-12 个月自然放养结果当成公开主证据，会混入“持续激活不足导致后期衰减”的影响。

所以目前这份公开版说明文档，把 3 个月作为：
